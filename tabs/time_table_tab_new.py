...
class TimeTableTab(QWidget):
    ...
    def __init__(self, parent, backend):
        ...
        self.courses_widgets = []  # List of lists: one list per course slot
    ...
    def show_current_result(self):
        ...
        self.page_input.setValue(self.backend.current_result_index + 1)
        # Note: backend.results holds one course id per time block slot.
        course_ids = self.backend.results[self.backend.current_result_index]
        # Create a list to hold the time block widgets for each course slot.
    ...
    def _create_time_table(self):
        ...
        for row, time_str in enumerate(self.times):
    ...
    def _create_nav_layout(self):
        ...
        choose_btn = QPushButton('Choose', self)  # New button for adding the current selection
        ...
        choose_btn.clicked.connect(self.choose_current_schedule)  # Connect to our new method
        ...
        nav_layout.addWidget(choose_btn)  # Add the Choose button here
    ...
    def choose_current_schedule(self):
        """
        When the user presses the Choose button, this method builds a schedule
        by iterating through each course slot (each list in self.courses_widgets),
        using the current index of the combo box (in the first time block widget)
        to choose the actual course id from the same_time_course_ids list.
        The resulting list of course ids is then added to self.backend.enrollment_results.
        """
        if self.backend.results == []:
            return
        chosen_schedule = []
        # Loop over each course slot's widgets.
        for slot_widgets in self.courses_widgets:
            if slot_widgets:
                # The first time block in the slot has the dropdown for selecting among alternatives.
                time_block = slot_widgets[0]
                # Use the current index from the dropdown if it exists (otherwise default to 0)
                current_index = time_block.dropdown.currentIndex() if hasattr(time_block, "dropdown") else 0
                # Use the same_time_course_ids list from the TimeBlock to get the actual course id.
                chosen_course_id = time_block.same_time_course_ids[current_index]
                chosen_schedule.append(chosen_course_id)
        # Append the chosen schedule to the backend enrollment_results.
        self.backend.enrollment_results.append(chosen_schedule)
        self.backend.enrollment_states.append([2] * len(chosen_schedule))
        if self.parent.enrollment_tab.update_enrollment_schedule_view():
            self.parent.enrollment_tab.show_current_schedule()
        # print("Chosen schedule added to enrollment_results:", chosen_schedule)
